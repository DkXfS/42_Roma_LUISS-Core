...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   create_elem_2_utils.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: apanthap <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/19 11:59:44 by apanthap          #+#    #+#             */
/*   Updated: 2021/04/19 11:59:46 by apanthap         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_camera_orientation(t_cam *cam)
{
	...
	else if (is_equal(v_norm(v_cross_prod(cam->direction,
					create_v(0, 1, 0))), 0))
		cam->up = create_v(1, 0, 0);
	else
	{
		cam->up = v_normalize(v_sub(project_p_to_plane(v_sum(cam->origin,
							create_v(0, 1, 0)), cam->origin, cam->direction),
					cam->origin));
...
void	set_square_orientation(t_square *square)
{
	...
	{
		p1 = project_p_to_plane(v_sum(square->c, create_v(0, 1, 0)),
				square->c, square->n);
...
void	set_cylinder_orientation(t_cyl *cylinder)
{
	...
	{
		p = project_p_to_plane(v_sum(cylinder->c, create_v(0, 1, 0)),
